...
def obtain_attach_nodes(args,node_idxs, labels, size):
    ...
    target_class = args.target_class
    target_idxs = [idx for idx, label in zip(node_idxs, labels) if label == target_class]
    size = min(len(target_idxs),size)
    rs = np.random.RandomState(args.seed)
    choice = np.arange(len(target_idxs))
    rs.shuffle(choice)
    selected_idxs = torch.tensor([target_idxs[i] for i in choice[:size]], device='cuda:0')
...
def cluster_distance_selection(args,data,idx_train,idx_val,idx_clean_test,unlabeled_idx,train_edge_index,size,device):
    ...
    print('encoder is on cpu now')
    kmedoids = cluster.KMedoids(n_clusters=nclass,method='pam')
    print('kmedoids is:',kmedoids)
    kmedoids.fit(encoder_x[seen_node_idx].detach().cpu().numpy())
    #kmedoids.fit(encoder_x[seen_node_idx].cpu().numpy())
    print('kmedoids is fitting')
