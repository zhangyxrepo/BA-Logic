...
parser.add_argument('--model', type=str, default='GAT', help='model',
                    choices=['GCN','GAT','GraphSage','GIN'])
parser.add_argument('--dataset', type=str, default='Pubmed', 
                    help='Dataset',
                    choices=['Cora','Citeseer','Pubmed','PPI','Flickr','ogbn-arxiv','Reddit','Reddit2','Yelp'])
parser.add_argument('--k', type=int, default=5, help='Number of top scores to evaluate')
...
parser.add_argument('--trigger_size', type=int, default=3,
                    help='tirgger_size')
parser.add_argument('--vs_size', type=int, default=320,
...
parser.add_argument('--selection_method', type=str, default='sort',
                    choices=['sort','conf','cluster','none','cluster_degree'],
                    help='Method to select idx_attach for training trojan model (none means randomly select)')
parser.add_argument('--test_model', type=str, default='GCN_metric',
                    choices=['GCN','GAT','GraphSage','GIN', 'GCN_metric'],
...
for seed in seeds:
    ...
    if(args.selection_method == 'none'):
        idx_attach = hs.obtain_attach_nodes(args, idx_train, data.y.cpu(), size)
    ...
    test_model.fit(poison_x, poison_edge_index, poison_labels, bkd_tn_nodes, idx_val,train_iters=args.epochs,verbose=False)

    output = test_model(poison_x,poison_edge_index)
    ...
    clean_acc = test_model.test(poison_x,induct_edge_index,data.y,idx_clean_test)
    ...
    if(args.evaluate_mode == '1by1'):
        ...
        for i, idx in enumerate(idx_atk):
            ...
            with torch.no_grad():
                ...
                output = test_model(induct_x,induct_edge_index)
        ...
        print("Flip ASR: {:.4f}/{} nodes".format(flip_asr,flip_idx_atk.shape[0]))
        ca = test_model.test(induct_x,induct_edge_index,data.y,idx_clean_test)
...
trojan_edge = model.get_trojan_edge(len(data.x),idx_attach,args.trigger_size).to(device)
node_index = idx_attach.tolist()
explanation = explainer(poison_x, poison_edge_index, index=node_index)
print(f'Generated explanations in {explanation.available_explanations}')
print(explanation.node_mask)
neighbor_scores = []
count = 0
total = 0
# trigger_nodes = model.get_trigger_nodes(data.x, idx_attach)
for v in idx_attach:
    v = v.unsqueeze(0)
    sub_nodes, *_ = k_hop_subgraph(node_idx=v, num_hops=1, edge_index=poison_edge_index)
    
    if sub_nodes.numel() == 0:
        print(f"No subgraph found for node {v}")
        continue
    ...
    trigger_nodes = torch.unique(trigger_nodes).to(device)
    
    # 计算 trigger_nodes 分数
    mask = torch.isin(sub_nodes, trigger_nodes)
    trigger_nodes = sub_nodes[mask]
    trigger_scores =explanation.node_mask[trigger_nodes].sum(dim=1)/ explanation.node_mask[trigger_nodes].shape[1]
    # print('Score of trigger nodes:', trigger_scores)
    
    # 计算 non-trigger nodes 的分数
    non_trigger_nodes = sub_nodes[~mask]
    non_trigger_scores =explanation.node_mask[non_trigger_nodes].sum(dim=1) 
    all_scores = torch.cat((trigger_scores, non_trigger_scores))
    if len(all_scores) < args.k:
        # print('Not enough neighbors to select top 3 and skip.')
        continue
    top_scores, _ = torch.topk(all_scores, args.k)
    
    is_trigger_in_top3 = trigger_scores in top_scores
    if is_trigger_in_top3:
        count += 1
    
    total += 1

if total > 0:
    ratio_in_top3 = count / total
    print('Ratio of trigger scores in top {}: {:.3f}'.format(args.k, ratio_in_top3))
else:
    print('No valid were performed.')
