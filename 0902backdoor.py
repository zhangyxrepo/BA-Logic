    ...
    def fit(self, features, edge_index, edge_weight, labels, idx_train, idx_attach, idx_unlabeled):
        ...
        for i in range(args.trojan_epochs):
            ...
            for j in range(self.args.inner):
                ...
                loss_inner = F.nll_loss(output[idx_tmp], labels[idx_tmp]) 
...
#%%         
            ...
            clip_grad_norm_(self.surrogate_model.parameters(), max_norm=5e-8)
            T = 5e-6
            non_trigger_grads = 0
            trigger_grads = 0
            ...
            loss_outter = loss_target + loss_logic
            ...
            self.poisoned_paras = self.surrogate_model.state_dict()
